...
from pyspark.sql.functions import to_date, to_timestamp, col, year, month, desc, rank, regexp_replace, when 
import time
...
crime_data_per_zip = df1.join(df_rev_ge_coding.hint('merge'),
...
top_victims = top_income_zip.join(crime_data_per_zip.hint("merge"), "Zip Code").orderBy(col("count").desc())
bottom_victims = bottom_income_zip.join(crime_data_per_zip.hint("merge"), "Zip Code").orderBy(col("count").desc())
