...
crime_data_per_zip = df1.join(df_rev_ge_coding.hint('shuffle_hash'),
...
top_victims = top_income_zip.join(crime_data_per_zip.hint('shuffle_hash'), "Zip Code").orderBy(col("count").desc())
bottom_victims = bottom_income_zip.join(crime_data_per_zip.hint('shuffle_hash'), "Zip Code").orderBy(col("count").desc())
