...
crime_data_per_zip = df1.join(broadcast(df_rev_ge_coding),
...
top_victims = top_income_zip.join(broadcast(crime_data_per_zip), "Zip Code").orderBy(col("count").desc())
bottom_victims = bottom_income_zip.join(broadcast(crime_data_per_zip), "Zip Code").orderBy(col("count").desc())
